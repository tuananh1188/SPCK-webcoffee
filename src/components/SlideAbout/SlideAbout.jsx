...
const slideDataAbout = [
    ...
    },
    {
        ...
        decription:
            'Mọi nụ cười ở coffee house đều bắt đầu tự hạt cà phê nguyên bản',
        routeText: '/about'
    }
];
...
const SlideAbout = () => {
    ...
    const goToPrev = () => {
        setCurrentIndex(
            prev => (prev - 1 + totalSlidesAbout) % totalSlidesAbout
        );
    };
    ...
    return (
        <div className='slider relative w-full h-150 m-auto overflow-hidden mt-10 '>
            <button
                onClick={goToPrev}
                className='absolute top-[50%] left-0 -translate-y-[30%]  bg-gray-950 opacity-60 text-amber-50 text-4xl py-2 ml-4 rounded-xl cursor-pointer hover:bg-amber-50 hover:text-[#EE7442] transition'
            >
                <LeftOutlined />
            </button>
            ...
            <button
                onClick={goToNext}
                className='absolute top-[50%] right-0 bg-gray-950 opacity-60 text-amber-50 -translate-y-4 text-4xl py-2 mr-4 rounded-xl cursor-pointer hover:bg-amber-50 hover:text-[#EE7442] transition'
            >
                <RightOutlined />
            </button>
        </div>
